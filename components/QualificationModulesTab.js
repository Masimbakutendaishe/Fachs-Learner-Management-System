...
export default function QualificationModulesTab({ programme }) {
  ...
  useEffect(() => {
    if (programme?.id) fetchAll();
  }, [programme?.id]);

  const fetchAll = async () => {
    setLoading(true);
    const { data: modulesData } = await supabase.from("qualification_modules").select("*").eq("programme_id", programme.id);
    const byType = {};
