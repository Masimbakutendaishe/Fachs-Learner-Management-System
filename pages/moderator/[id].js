...
export default function ModeratorProgrammeReview() {
  ...
  const fetchAll = async () => {
    ...
    const { data: subsData } = await supabase
      ...
      .select(`
        ...
        moderator_comment, moderator_agrees_with_marks, moderator_signed_off, moderator_signed_at,
        profiles ( first_name, surname ),
        unit_weeks ( unit_standard_title, week_end_date )
      `)
      .eq("programme_id", id)
      .eq("assessor_signed_off", true)
      .order("assessor_signed_at", { ascending: false });
  ...
  if (!programme) return <div className="paper p-8 text-center text-gray-500 text-sm">Not found, or you're not assigned as moderator for this qualification.</div>;

  const filtered = submissions.filter((s) => {
    if (filter === "awaiting") return !s.moderator_signed_off;
    if (filter === "reviewed") return s.moderator_signed_off;
    return true;
  });

  return (
    <div className="animate-fade-up">
      <Link href="/moderator/dashboard" className="flex items-center gap-1.5 text-sm text-[var(--text-muted)] hover:text-[var(--text)] mb-4 w-fit">
      ...
      {tab === "review" && (
        <>
          <div className="flex gap-2 mb-6">
            {[
              { key: "awaiting", label: "Awaiting Moderation" },
              { key: "reviewed", label: "Moderated" },
              { key: "all", label: "All" },
            ].map((f) => (
              <button
                key={f.key}
          ...
          {filtered.length === 0 ? (
            ...
            <div className="space-y-4">
              {filtered.map((sub) => (
                <ModeratorSubmissionRow key={sub.id} sub={sub} supabase={supabase} onUpdated={fetchAll} />
              ))}
            </div>
          )}
        </>
...
function ModeratorSubmissionRow({ sub, supabase, onUpdated }) {
  const [comment, setComment] = useState(sub.moderator_comment || "");
  const [agrees, setAgrees] = useState(sub.moderator_agrees_with_marks ?? null);
  ...
  const handleSignOff = async () => {
    if (agrees === null) return alert("Confirm whether you agree with the marks first.");
    setSaving(true);
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          moderator_agrees_with_marks: agrees,
          moderator_signed_off: true,
          moderator_signed_by: user.id,
          moderator_signed_at: new Date().toISOString(),
        })
        .eq("id", sub.id);
    ...
    <div className="paper p-5">
      <div className="flex items-start justify-between gap-3 mb-3 flex-wrap">
        <div>
          ...
          <p className="text-xs text-gray-400 font-mono">Assessor signed off {new Date(sub.assessor_signed_at).toLocaleDateString()}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {sub.moderator_signed_off && (
            <span
              className="text-xs font-medium px-2.5 py-1 rounded-full"
              style={sub.moderator_agrees_with_marks ? { background: "#ECFDF5", color: "#047857" } : { background: "#FEF2F2", color: "#B91C1C" }}
            >
              {sub.moderator_agrees_with_marks ? "Agreed" : "Disputed"}
            </span>
          )}
        </div>
      </div>

      {sub.assessor_comment && (
        <p className="text-xs text-gray-500 mb-3 italic">Assessor's comment: {sub.assessor_comment}</p>
      )}
      ...
      {!sub.moderator_signed_off && (
        <div className="flex gap-3 mb-3">
          <button
            ...
            style={agrees === true ? { background: "#ECFDF5", color: "#047857", border: "1px solid #047857" } : { background: "white", border: "1px solid var(--border-soft)", color: "var(--text)" }}
          >
            Marks are agreeable
          </button>
          <button
            ...
            className="flex-1 py-2 rounded-lg text-sm font-medium"
            style={agrees === false ? { background: "#FEF2F2", color: "#B91C1C", border: "1px solid #B91C1C" } : { background: "white", border: "1px solid var(--border-soft)", color: "var(--text)" }}
          >
            Marks are disputed
          </button>
        </div>
      ...
      <textarea
        placeholder="Moderator comment..." value={comment} onChange={(e) => setComment(e.target.value)}
        disabled={sub.moderator_signed_off} rows={2}
