...
const ResourceCard = ({
  ...
  return (
    <div className="p-6 bg-white shadow-lg rounded-lg mb-4 text-gray-900">
      ...
      {instructions && (
        <p className="text-gray-900 text-sm mb-2">{instructions}</p>
      )}
      ...
      {allowTextAndFile ? (
        ...
        <>
          <p className="font-medium text-gray-900 mb-2">{questions[step]}</p>
          ...
          <div className="flex justify-between mb-2">
            ...
            {step < questions.length - 1 ? (
              ...
              <button
                onClick={() => alert("All files uploaded (dummy)!")}
        ...
        <input
          ...
          className="mt-2 border p-2 rounded bg-white text-gray-900"
        />
      )}
    </div>
  );
...
/* ---------- Reusable Upload Menu ---------- */
const UploadMenu = ({ label }) => {
      ...
      {mode === "upload" && (
        <div className="mt-4">
          ...
          <div className="flex gap-2 mt-2">
            <button
              onClick={() => alert("File uploaded (dummy)!")}
              className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
            >
              Submit
            </button>
        ...
        <div className="mt-4 space-y-4">
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              onChange={(e) =>
                setCurrentQ((q) => ({ ...q, marks: e.target.value }))
              }
              placeholder="Marks"
            ...
            <div className="mt-4">
              ...
              <ul className="space-y-2">
                {questions.map((q, i) => (
                  <li
                    key={i}
                    className="flex justify-between items-center border-b pb-1 text-gray-900"
                  >
                    <span>
                      {i + 1}. {q.text} — <span className="font-bold">{q.marks} marks</span>
                    </span>
                    <button
              ...
              <button
                onClick={() => alert("Questions saved (dummy)!")}
...
const AttendanceMenu = () => (
  <div className="p-6 bg-white shadow-lg rounded-lg mb-4 text-gray-900">
    ...
    <button
      className="mt-4 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700"
      onClick={() => alert("Attendance marked (dummy)!")}
    >
      Submit Attendance
    </button>
  </div>
...
const ScheduleTeamsMenu = () => (
  <div className="p-6 bg-white shadow-lg rounded-lg mb-4 text-gray-900">
    ...
    <label className="block mb-2 text-gray-900">
      Topic:
      <input type="text" className="border p-2 rounded w-full mt-1 bg-white text-gray-900 placeholder-gray-500" />
    </label>
    <button
      className="mt-2 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
      onClick={() => alert("MS Teams session scheduled (dummy)!")}
    >
      Schedule
    </button>
  </div>
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 p-8 overflow-y-auto h-full">
        ...
        {currentActivity === "attendance" && <AttendanceMenu />}
        ...
        {currentActivity === "schedule_teams" && <ScheduleTeamsMenu />}
      </main>
    </div>
  );
}
